...
class DatabaseManager:
    ...
    def incluir_cliente(self, cliente):
        ...
        try:
            sql = """
            INSERT INTO users(nome,email,telefone,endereco,cidade,estado,cep,datacadastro,datanascimento)
                      VALUES(?,?,?,?,?,?,?,?,?) """
    ...
    def validar_cliente(self, cliente):
        if any(
            [
                ...
                not cliente.endereco,
                not cliente.cidade,
                not cliente.estado,
                not cliente.cep,
                not cliente.datacadastro,
                not cliente.datanascimento,
            ]
        ):
            return False
